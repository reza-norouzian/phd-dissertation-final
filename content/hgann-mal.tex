...
\paragraph{Comparison scope.} Common call graphs and node features support the comparison of
graph and hypergraph models under shared evaluation partitions. The measured gains concern
the complete \hgann{} configuration; the individual contribution of attention remains a
question for the ablations in Section~\ref{sec:hgann:13}. The baselines assess general learning
operators within this pipeline. Comparisons with dedicated Android detectors would extend
this evidence to a broader range of detection systems.

\paragraph{Evaluation coverage.} Drebin and CICMalDroid provide distinct application collections
and classification tasks. The five-run means describe performance on fixed random partitions;
individual run scores are unavailable for quantifying variability. Evaluation on later
application releases would require time-ordered splits \cite{pendlebury2019tesseract}.
Grouping repackaged applications before partitioning would also address the potential overlap
between training and test code identified in Section~\ref{sec:hgann:9}.

\paragraph{Static evidence and robustness.} Package-based analysis supports classification before
execution, with coverage determined by the bytecode and call targets recovered by the analyser
(Section~\ref{sec:hgann:3}). Additional runtime or native-code analysis could broaden this
coverage. The experiments evaluate applications as collected; dedicated tests of obfuscation
and detector-directed evasion remain future work. Published attacks on call-graph detectors
provide relevant candidates for such tests
\cite{zhao2021structural,song2026fcghunter,li2025efficient}. Their application to \hgann{}
would assess robustness under the explicit adversary assumptions described in
Chapter~\ref{chap:advml}.

\section{Chapter Summary}
\label{sec:hgann:15}

This chapter addressed RQ2 through \hgann{}, which represents groups of related methods within
an Android application. Hyperedges capture call-graph proximity and similarity between method
representations. Slice-aware groups add context around security-sensitive API calls, and
attention learns how strongly each method participates in its groups. The resulting application
representation supports binary detection and prediction of family or category labels from
static code. The mathematical analysis established properties of this attention operator and
bounds for storage and sparse propagation.

The evaluation compared graph and hypergraph models on Drebin and CICMalDroid under shared
fixed partitions. Across the four task--dataset combinations, every hypergraph configuration
exceeded the stronger pairwise baseline by 3.1 to 10.8 percentage points in mean accuracy over
five training seeds. \hgann{} exceeded the strongest non-attentive hypergraph baseline in
three of these settings, with accuracy gains of 1.2 to 4.1 points. On CICMalDroid binary
detection, HGNN+ achieved 97.7\% accuracy and \hgann{} achieved 97.6\%. The largest gains for
\hgann{} occurred in CICMalDroid category classification.

These results support higher-order modelling of static program context within the evaluated
configurations. Section~\ref{sec:hgann:13} identifies the experiments needed to measure the
separate contributions of construction and attention. In the dissertation, \hgann{} complements
\hybroid{} through its focus on relations within the application package, while \hybroid{}
combines program structure with observed network traffic. Chapter~\ref{chap:discussion}
examines what these two approaches contribute to contextual security analysis.
