...
Construction and aggregation change together, so their individual contributions remain
unresolved. Section~\ref{sec:hgann:13} specifies the ablations needed to separate them.
...
Sections~\ref{sec:hgann:4} to~\ref{sec:hgann:8} describe the method, and
Sections~\ref{sec:hgann:9} to~\ref{sec:hgann:13} report the evaluation and analysis.
...
Across the evaluated tasks, the hypergraph configurations achieved higher reported accuracy
than the pairwise baselines. \hgann{} exceeded the strongest non-attentive hypergraph baseline
...
publication reports none of these tests. The first two comparisons address RQ3; the others
concern the contributions of individual families and the sensitivity to construction parameters.
...
In the reported comparisons, all hypergraph configurations achieved higher accuracy than the
pairwise baselines. \hgann{} exceeded the strongest non-attentive hypergraph baseline in three
of four task--dataset combinations; on CICMalDroid binary detection, its accuracy was
97.6 per cent compared with 97.7 per cent for HGNN+. The operator analysis in
Section~\ref{sec:hgann:8} describes representational capacity separately from these empirical
comparisons. Attention reweights the
memberships that the construction fixed, and within one layer it ranks the hyperedges of every
method in one shared order. It can distinguish hyperedge groupings that produce the same fixed
propagation matrix, although the experiments do not establish whether training uses this capacity.
