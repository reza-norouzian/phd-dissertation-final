...
\begin{figure}[tbp]
  ...
  HGNNP is the source publication's name for HGNN+.}
  \label{fig:hgann:f1}
\end{figure}
...
97.8\% in the test cohort. This is a conditional calculation, not a measured deployment
result. It shows why the precision of malware alerts should be interpreted together with the
malware share in the population being analysed.
...
\begin{table}
  \caption{Multiclass classification results (\%). Each F1, precision and recall value is a
...
the trivial baseline the margin is wide here, since the largest family covers 19.8\% of the
samples.
