...
Section~\ref{sec:hgann:14} discusses the resulting threats to validity.
...
\begin{table}
  \caption{Binary detection, in per cent, as published \cite{norouzian2025hgannmal}. The source
  labels the F1 column Macro-F1 without specifying an averaging convention. The empirical
  \hgann{} Drebin matrix gives a positive-class F1 of 97.7 per cent
  (Section~\ref{sec:hgann:10}); the published 97.8 is retained here.
  Section~\ref{sec:hgann:14} discusses the metric inconsistencies. Best values in bold.}
...
\begin{figure}[tbp]
  ...
  tasks, as published in Tables~\ref{tab:hgann:binary} and~\ref{tab:hgann:multi}. The plot retains
  the published values, including the Drebin binary discrepancy described in
  Section~\ref{sec:hgann:10}. The source labels them Macro-F1, although several rows cannot follow
  one averaging convention (Section~\ref{sec:hgann:14}). The vertical axis is truncated at 80
  so that differences among the hypergraph methods remain visible. HGNNP is the source
  publication's name for HGNN+.}
  \label{fig:hgann:f1}
\end{figure}
...
baseline by 3.1 to 10.8 points of reported accuracy. \hgann{} exceeded the strongest non-attentive hypergraph baseline
...
On CICMalDroid binary detection, \hgann{} achieved 97.6 per cent accuracy compared with
97.7 per cent for HGNN+. These single-run estimates do not establish statistical equivalence
or a reliable performance difference.
...
give a false-positive rate of 1.3 per cent and a positive-class F1 of 97.7 per cent. The published
F1 is 97.8 per cent; Table~\ref{tab:hgann:binary} and Figure~\ref{fig:hgann:f1} retain that source
value, which differs from the value calculated from the empirical matrix by 0.1 percentage point
after rounding. Precision, recall and accuracy calculated from the matrix round to the published
97.8, 97.6 and 98.3 per cent, respectively.
...
\begin{table}
  \caption{Multiclass classification, in per cent, as published \cite{norouzian2025hgannmal}.
  The Drebin task assigns a malicious application to one of twenty families; the CICMalDroid task assigns an application to
...
test-set counts. The published aggregate scores do not provide this information, and their
averaging convention remains unresolved (Section~\ref{sec:hgann:14}).
...
published F1 margins in Figure~\ref{fig:hgann:f1} are 4.7 points here, 1.9 and 1.6 on the Drebin family and
...
\paragraph{Threats to validity.} The source labels F1 as Macro-F1 without specifying an averaging
convention. For the \hgann{} Drebin binary row a macro reading is impossible: over 1,112
malicious and 1,894 benign test applications, macro precision 97.8 and macro recall 97.6 at an
accuracy that rounds to 98.3 would require at least 1,897 true negatives. Precision and recall
match the positive-class rates of the empirical matrix; the published F1 does not
(Section~\ref{sec:hgann:10}). Under positive-class, macro and support-weighted averaging alike,
...
the stronger pairwise baseline by 3.1 to 10.8 points of accuracy. \hgann{} exceeded the
strongest non-attentive hypergraph baseline in three of four task--dataset combinations; on
CICMalDroid binary detection, its accuracy was 97.6 per cent compared with 97.7 per cent for
HGNN+.
