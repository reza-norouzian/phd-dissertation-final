...
% T-022: priorities 1, 4 and 5 of T-020 only, approved on 20 September 2026.
% Reconcile published/matrix-derived F1, refine prose and bound literature claims.
% Preserve method specifications, equations, table bodies and figure assets.
% T-024: approved priority-3 interpretation passages, 20 September 2026.
% Method specifications remain unchanged; numerical results retain their provenance.
% T-031 and D-007: RQ3 framing, results statement and summary aligned with the revised question.
% T-032: author-corrected five-run macro means on fixed splits.

\section{Research Problem and Relation to Hybroid}
\label{sec:hgann:1}

This chapter answers RQ3, stated in Section~\ref{sec:intro:rqs}: \rqThreeText{} It addresses
gap~\gapref{3}. Studies have quantified the information lost when group relations are projected
...
Sections~\ref{sec:hgann:4} to~\ref{sec:hgann:8} answer the first part of RQ3 through the
...
\hgann{} analyses the manifest and Dalvik bytecode of an application package without executing
the application. These inputs determine which parts of its behaviour can be represented.

Static analysis removes the need for a device or emulator and for scripted interaction. Packages
can be analysed independently across a corpus; Section~\ref{sec:hgann:13} treats the computational
cost of representation construction and learning. The second modality of
Chapter~\ref{chap:hybroid} depends on traffic that the CICAndMal2017 authors captured on physical handsets, and
Section~\ref{sec:discussion:3} compares the two observation requirements directly.

The representation inherits the static-analysis limits described in
Section~\ref{sec:background:android:analysis}. Code loaded at runtime is absent from the
recovered bytecode. Reflective calls can also be missing because the pipeline does not model
their target resolution. Packed or native payloads may lie outside the parsed DEX files.
These omissions restrict the methods and relations available to hypergraph construction.

The evaluation uses historical samples without detector-directed modification, so its threat
model is non-adaptive. Rewriting an application can change the recovered call graph and the
representations derived from it. Section~\ref{sec:hgann:14} relates this exposure to published
attacks on function-call-graph detectors; Chapter~\ref{chap:advml} treats evaluation under
explicit adversary assumptions.
...
arithmetic means over these five runs, as defined in
Equation~\ref{eq:background:run-aggregation}. These repetitions sample training randomness
...
For the first part of RQ3, the chapter lifted a method-level function-call graph to a
