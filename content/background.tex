...
Since Android~5 that bytecode is compiled ahead of time and executed by the Android Runtime
rather than interpreted by Dalvik \cite{aosp2026art}, although the instructions an analyst reads
are the same in either case. Section~\ref{sec:hybroid:5} depends on this distinction,
because the operand values attached to an instruction vary with the runtime while the opcode
does not.
...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}[
      ...
      box/.style={draw, rounded corners=2pt, minimum height=15pt, font=\scriptsize, inner sep=3pt},
      minor/.style={draw, dashed, rounded corners=2pt, minimum height=13pt, font=\scriptsize, inner sep=2.5pt, text=black!55},
      note/.style={font=\scriptsize\itshape, text=black!70},
      band/.style={draw, rounded corners=3pt, font=\scriptsize}
    ]
    % ---------------- left: the package ----------------
    \node[font=\small\bfseries] at (1.75,4.30) {the package};
    \node[draw, thick, rounded corners=3pt, minimum width=3.6cm, minimum height=3.7cm]
      (apk) at (1.75,2.20) {};
    \node[note] at (1.75,0.10) {ZIP archive};
    \node[box, minimum width=3.0cm] (man) at (1.75,3.55) {\texttt{AndroidManifest.xml}};
    \node[note, align=center, text width=3.1cm] at (1.75,3.05)
      {components, permissions,\\filtered intents};
    \node[box, minimum width=3.0cm] (dex) at (1.75,2.35) {\texttt{classes*.dex}};
    \node[note, align=center, text width=3.1cm] at (1.75,1.85)
      {Dalvik bytecode: methods,\\invocations, basic blocks};
    \node[minor, minimum width=3.0cm] (res) at (1.75,1.10) {\texttt{res/}\quad\texttt{assets/}\quad\texttt{lib/}};
    % ---------------- right: fields of view ----------------
    \node[font=\small\bfseries] at (8.30,4.30) {what each mode observes};
    \node[band, fill=black!6, minimum width=5.9cm, align=left, inner sep=5pt,
          text width=5.4cm] (st) at (8.30,3.35)
      {\textbf{static}\ \ all shipped code, run or not\\
       \textcolor{black!60}{not reached: packed and runtime-loaded code}};
    \node[band, fill=black!12, minimum width=5.9cm, align=left, inner sep=5pt,
          text width=5.4cm] (dy) at (8.30,2.20)
      {\textbf{dynamic}\ \ paths the harness triggers\\
       \textcolor{black!60}{adds: runtime values, unpacked code, traffic}};
    \node[band, dashed, minimum width=5.9cm, align=left, inner sep=5pt,
          text width=5.4cm] (hy) at (8.30,1.25)
      {\textbf{hybrid}\ \ the union of the two, no new primitive};
    \draw[->, thin] (apk.east) -- (st.west);
    \draw[->, thin] (apk.east) -- (dy.west);
  \end{tikzpicture}
...
Figure~\ref{fig:background:hypergraph} shows both the lifting and what the projection costs.

\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[
      every node/.style={font=\small},
      vtx/.style={circle, draw, fill=white, inner sep=1.2pt, minimum size=13pt, font=\scriptsize},
      hedge/.style={draw, thick, rounded corners=9pt},
      lbl/.style={font=\scriptsize\itshape},
      pnl/.style={font=\small\bfseries}
    ]
    % ---- shared vertex layout, identical in all three panels ----
    \def\vlayout{
      \node[vtx] (m1) at (0.00,1.30) {$m_1$};
      \node[vtx] (m2) at (0.95,1.75) {$m_2$};
      \node[vtx] (m3) at (1.05,0.75) {$m_3$};
      \node[vtx] (m4) at (2.05,1.20) {$m_4$};
      \node[vtx] (m5) at (2.10,0.20) {$m_5$};
      \node[vtx] (m6) at (1.15,-0.30) {$m_6$};
    }
    % ================= panel (a): hypergraph =================
    \begin{scope}[xshift=0cm]
      \begin{scope}[on background layer]
        \draw[hedge, fill=black!8]
          ($(0.00,1.30)+(-0.42,0.10)$) -- ($(0.95,1.75)+(0.05,0.42)$)
          -- ($(1.05,0.75)+(0.30,-0.34)$) -- cycle;
        \draw[hedge, fill=black!14]
          ($(1.05,0.75)+(-0.34,0.28)$) -- ($(2.05,1.20)+(0.34,0.30)$)
          -- ($(2.10,0.20)+(0.28,-0.36)$) -- cycle;
        \draw[hedge, fill=black!5]
          ($(2.10,0.20)+(0.40,-0.02)$) -- ($(1.15,-0.30)+(-0.40,-0.16)$)
          -- ($(1.15,-0.30)+(-0.10,0.34)$) -- ($(2.10,0.20)+(0.10,0.36)$) -- cycle;
      \end{scope}
      \vlayout
      \node[lbl] at (0.15,2.35) {$e_1,\ \delta=3$};
      \node[lbl] at (2.75,1.60) {$e_2,\ \delta=3$};
      \node[lbl] at (2.30,-0.55) {$e_3,\ \delta=2$};
      \node[pnl] at (1.05,-1.15) {(a) hypergraph};
    \end{scope}
    % ================= panel (b): clique expansion =================
    \begin{scope}[xshift=5.0cm]
      \vlayout
      \draw (m1)--(m2); \draw (m2)--(m3); \draw (m1)--(m3);
      \draw (m3)--(m4); \draw (m4)--(m5); \draw (m3)--(m5);
      \draw (m5)--(m6);
      \node[pnl] at (1.05,-1.15) {(b) clique expansion};
    \end{scope}
    % ================= panel (c): what cannot be recovered =========
    \begin{scope}[xshift=10.0cm]
      \vlayout
      \draw[very thick] (m1)--(m2); \draw[very thick] (m2)--(m3); \draw[very thick] (m1)--(m3);
      \draw (m3)--(m4); \draw (m4)--(m5); \draw (m3)--(m5);
      \draw (m5)--(m6);
      \node[lbl, align=center] at (0.30,2.45) {one hyperedge\\or three edges?};
      \draw[->, thin] (0.55,2.10) -- (0.72,1.55);
      \node[pnl] at (1.05,-1.15) {(c) the ambiguity};
    \end{scope}
  \end{tikzpicture}
  \caption[Pairwise projection of a hypergraph]{The same six methods as a hypergraph and as its
  clique expansion. Hyperedge $e_3$ joins two vertices, so it is an ordinary graph edge: every
  graph is a hypergraph. Panel~(c) marks what the projection costs. The triangle over
  $m_1, m_2, m_3$ arises both from the single hyperedge $e_1$ and from three separate pairwise
  relations among the same methods, and the projected graph does not distinguish the two. The
  joint membership of $m_3$ in $e_1$ and $e_2$ is likewise no longer readable. The projection
  preserves which methods are related and loses that they were related as one group
...
\section{Evaluation Methodology and Experimental Bias}
\label{sec:background:evaluation}

Chapter~\ref{chap:introduction} reported what happens to published performance under a stricter
protocol. This section states the mechanisms behind those results and fixes the standard this
dissertation holds itself to.
...
\paragraph{What a score means.} Axelsson derived the constraint that binds every detector
deployed against a rare event: because the base rate of intrusions is low, the probability that
an alarm signals a genuine intrusion is dominated by the false-alarm rate, and a detector must
therefore reach a false-alarm rate that is difficult to attain before its alarms carry
information \cite{axelsson2000base}. Accuracy and precision are consequently uninterpretable
without the class ratio that produced them \cite{arp2022dos}, and a reported accuracy is read
against the floor set by a majority-class or constant-output predictor. In time-series detection a further convention
distorts comparison: \emph{point adjustment} marks an entire anomalous window as detected once
...
\phantomsection
\paragraph{G2. What combining program structure with observed behaviour buys.}\label{gap:2} Static and
dynamic analysis are usually set against one another as alternatives. Where they have been
measured under one protocol, static analysis proved at least as effective as dynamic analysis
and the combination measured best \cite{onwuzurike2018family}, but that comparison has rarely
been repeated, and hybrid systems are seldom evaluated against each modality alone on the same
samples \cite{lindorfer2015marvin}. The gain attributable to the combination, as distinct from
the gain attributable to either part, is therefore not established. RQ2 takes up this question.
