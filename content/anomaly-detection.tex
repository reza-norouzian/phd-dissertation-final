...
This chapter answers RQ3.1 and RQ3.2, the two subquestions of RQ3 stated in
Section~\ref{sec:intro:rqs}, and takes up gap~\gapref{3}. Three pieces of work support the
...
network and process data, together with its S7comm and PROFINET extractor and its integration in
...
or from conventional network traffic. The SWaT path does not model communication. It consumes the sensor and actuator
...
The studies use different decision rules. The service-graph method
(Section~\ref{sec:anomaly:service-graphs}) stores the inter-service relations observed during its
learning phase and flags a later relation that is absent from the graph. The GAN study
(Section~\ref{sec:anomaly:gan}) uses mixed normal and attack windows for fitting, combines
reconstruction and critic scores, and applies an adaptive threshold that assigns a binary
prediction to each timestamp. Both methods depend on representative fitting data: an attack
present during learning may be incorporated into the learned representation. The \nadics{}
evaluation on the S7-300 corpus (Section~\ref{sec:anomaly:iuno-corpus}) uses labelled
classification instead, so it addresses a different decision problem.

\section{Communication Graphs for IoT Services}
\label{sec:anomaly:service-graphs}

An IoT site consists of distributed computing nodes, and the devices on these nodes are
managed by microservices. The services cooperate to implement a use case. A heating controller,
for example, reads temperatures from distributed meters and sets the valves of the heating
devices \cite{aubet2018graph}. IoT data exchange is usually protected by access policies such as
OAuth tokens or group-based access control, and such policies hold only if every service
implements them correctly. Services also cooperate ad hoc on heterogeneous nodes. A site owner
therefore cannot keep track of the communication relations between services by hand
\cite{aubet2018graph}.

The problem addressed here has two parts: to build a model of these relations without manual
configuration, and to use it to detect anomalous traffic. Communication whitelisting is the
established approach in control networks (Section~\ref{sec:background:networks:industrial}).
Flow whitelists were built for SCADA networks \cite{barbosa2013flow}, and communication
patterns were later modelled per host pair \cite{yoon2014communication}. Both assume that the
topology stays stable after a single learning period. An IoT deployment changes whenever a
service is added or removed, so its model must allow revision during operation
\cite{aubet2018graph}.

The proposed model is compact. Services are vertices, observed relations are edges, and each
node maintains its own graph. A learning phase runs when a service starts and labels every
relation observed during that period as normal. A relation absent from the model later raises a
notification. The site owner may legitimate it, after which the system relabels the edge as
normal. Detection runs on the node hosting the service, so no central collection is needed
\cite{aubet2018graph}. The design also aimed to store the learned relations in the service
metadata, so that a service moved to another node would carry its model with it. Because the
implementation identifies services by concrete local addresses, this portability was not
reached; site-independent identities would have required semantic service discovery.

Two costs were reported. Memory per vertex is 100 bytes plus 88 bytes for every incident edge,
and a factor of 1.33 for the stacked hash tables enters once per vertex and once over the vertex
set. Twenty services with ten relations each thus occupy 33.7~KB. Detection adds about 6~ms to
an inter-service round trip of 24~ms, measured over ten to forty services \cite{aubet2018graph}.
...
to roughly 27~ms at forty, and every configuration has a long upper tail past the 160~ms axis
limit. The study states neither the repetition count nor the statistic behind the 6~ms and
24~ms values, which are best read as an aggregate over the four configurations. The added cost
does not grow between ten and forty services. Panel~(b) covers one run of about fourteen
minutes. Learning phases cluster in the first three minutes as services start, and another
follows at minute seven when a service is added. The graph then settles at fourteen vertices and
ten edges. One anomalous interval appears shortly after minute ten, and the edge count returns
to its earlier value once it ends, so the trace records the detection but no legitimation.

To summarise, the study shows that a node-local communication graph can be built without manual
configuration and revised by the site owner at run time, at a latency cost of about 6~ms and a
memory cost of some tens of kilobytes. Detection quality was not measured. No labelled attack
set was used, and the anomalous interval holds a previously unseen connection rather than an
executed attack, so the study reports no detection rate and no false-positive rate. A whitelist
also emits a verdict rather than a score, so even a labelled trace would yield no threshold
curve. RQ3.1 is therefore answered at the level of representation, update procedure, and
measured overhead.
...
depends on the security policy and the network layout of the individual site. The S7comm and
...
S7comm traffic \cite{kleinmann2014accurate} with a control server, and the traces were labelled
both in binary form and by attack class \cite{iuno_ap4}. In the second stage, the public analysers TShark, Bro-IDS and
Argus were combined with dissectors written for S7comm and PROFINET
\cite{akerberg2009exploring}, and the TUM feature extractor reduced each network connection to
...
The artefact behind RQ3.1 is a two-page contribution to PAM 2018 on which the candidate is the
...
implemented. RQ3.1 therefore carries no measured detection result.
...
RQ3.1 asks how service relationships can be represented as communication graphs. The reported
...
RQ3.2 asks how a modular framework can analyse network and process features for anomaly
...
Together, these mechanisms close gap~\gapref{3} at the implementation level. No study shows
