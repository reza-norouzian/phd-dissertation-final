...
\begin{sidewaysfigure}
  \centering
  \includegraphics[width=0.92\textheight,height=0.85\textwidth,keepaspectratio]{iuno-VW_testbed.pdf}
...
the five attack labels, an attack share of 4.54\%. Table~\ref{tab:anomaly:iuno-corpus}
...
Appendix~\ref{app:nadics-importance} lists all 63. \texttt{s7\_function} leads at 11.9\%,
because a stop or a start is an ordinary S7 operation whose function code is visible in the
request, and \texttt{role\_violation} follows at 8.0\%, because a write from a host whose
...
Protocol semantics holds the largest share at 36.4\%, although it also holds the most
fields. The comparison that carries information is the share per field: the nine relational
fields average 2.38\% each, against 0.74\% for the eighteen flow fields and 0.60\% for the five
...
S7 attack from S7 polling. Panel~(c) gives the cumulative share. Ten fields reach 54.5\%
of the total importance, twenty-four reach 80\% and thirty-six reach 90\%, so the
...
99.30\%, attack precision 92.94\% and attack recall 96.11\%, for an attack
$F_1$ of 94.50\%. The false-positive rate over normal records is 0.484\% and the
balanced accuracy 97.81\%. On the six-class task, accuracy reaches 98.85\%, the
macro-$F_1$ 89.56\% and the weighted $F_1$ 98.86\%.

Accuracy is the least informative of these numbers. Normal records make up 93.79\% of the
test partition, so a detector that never fired would already score 93.79\%, and the gap
between the weighted and the macro $F_1$, 98.86\% against 89.56\%, measures how much of the
six-class figure the normal class carries. Per-class recall is highest for denial of service at
95.19\% and lowest for brute force at 82.35\%, with authentication bypass close
behind at 83.91\%. Most errors of these two classes fall on other attack classes.
...
record-level false-positive rate of 0.128\%. Positives were merged into alarm episodes
...
No attack was executed during this period, so every alarm it raised was false. Its 0.128\%
record rate lies below the 0.484\% of the test session, whose normal records also include the
recovery intervals of executed attacks. The recovered causes share a structure that
...
were presented in 599.87 seconds and 361 were dropped, a loss of 0.024\% at a processed
rate of 2,497.38 frames per second. This input rate is about 17 times the average frame rate of
the corpus capture. The extractor used 43.1\% of the CPU capacity of the device on average
...
accumulates. The light load lost four of its 599,827 frames at 22.6\% CPU utilisation.
The overload row does not describe a supported rate: 11.71\% of the input was dropped,
and the processed rate stayed at 3,090.93 frames per second against an input rate of 3,500.69.
Mean CPU utilisation reached 50.4\% there, and the 95th-percentile emission delay rose to
...
and 342,226 normal timestamps. The measures follow the definitions in
Section~\ref{sec:background:metrics:binary}, with an anomalous timestamp as the positive class.
All table entries are calculated from integer confusion counts and rounded to two decimal
places.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    ...
    \midrule
    All 51 variables           & 70.22 & 94.34 & 80.51 & 6.02 \\
    PCA: 5 components          & 75.20 & 86.54 & 80.47 & 4.29 \\
    PCA: 24 components         & 73.11 & 91.89 & 81.43 & 5.08 \\
    Autoencoder representation & 72.05 & 86.32 & 78.54 & 5.03 \\
    Selected feature subset    & 70.91 & 84.10 & 76.94 & 5.19 \\
    \bottomrule
  \end{tabular}
\end{table}

The complete representation has the highest recall, at 94.34\%, together with the
highest false-positive rate, at 6.02\%. This profile represents a detector that identifies a
larger proportion of anomalous observations while also flagging more normal observations. The
five-component PCA configuration has the highest precision, at 75.20\%, and the lowest
false-positive rate, at 4.29\%, accompanied by lower recall of 86.54\%. In these results,
stronger compression therefore corresponds to fewer false-positive timestamps and more
missed anomalous timestamps.

The 24-component PCA configuration provides an intermediate balance, with precision of 73.11\%,
recall of 91.89\% and an $F_1$ score of 81.43\%. Relative to the complete representation, its
precision is 2.89 percentage points higher and its recall is 2.45 percentage points lower. Its
$F_1$ score is approximately 0.92 percentage points higher. This configuration has the highest
...
The autoencoder representation has a precision of 72.05\% and recall of 86.32\%,
corresponding to an $F_1$ score of 78.54\%. The selected feature subset has precision of
70.91\%, recall of 84.10\% and $F_1$ of 76.94\%. These values show lower detection coverage
...
Table~\ref{tab:anomaly:gan-confusion}: 47,279 of the 51,453 anomalous timestamps are detected
and 17,391 of the 342,226 normal timestamps are flagged as anomalous.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    Anomalous & 47,279 (TP) &   4,174 (FN) &  51,453 \\
    Normal    & 17,391 (FP) & 324,835 (TN) & 342,226 \\
    \midrule
    Total     & 64,670      & 329,009      & \textbf{393,679} \\
    \bottomrule
  \end{tabular}
\end{table}

From these counts, precision is 47,279/64,670 = 73.11\% and recall is 47,279/51,453 =
91.89\%. $F_1$ equals 94,558/116,123 = 81.43\%, the false-positive rate 17,391/342,226 =
5.08\%, and accuracy 372,114/393,679 = 94.52\%.

The recall of 91.89\% means that the detector flags approximately 92 of every 100 anomalous
...
different distributions of detection across attack episodes. Precision of 73.11\% means that
approximately 27\% of flagged timestamps fall within recorded normal periods. This proportion
differs from the false-positive rate of 5.08\%, whose denominator includes all normal
timestamps.
...
86.93\% accuracy while detecting no anomalous timestamps. The measured accuracy of 94.52\%
...
The 24-component PCA configuration combines the highest measured $F_1$ of 81.43\% with a
reduction from 51 input variables to 24 components. It detects 91.89\% of anomalous timestamps
at a false-positive rate of 5.08\%. Within this SWaT evaluation, these results support PCA as
a compact representation with a useful balance between detection coverage and false alarms.

The full-input and five-component PCA configurations have similar $F_1$ scores but different
error profiles. The full input detects a greater proportion of anomalous observations, while
five-component PCA flags fewer normal observations. Representation choice therefore offers a
practical way to address different monitoring priorities: broader detection coverage when
missed anomalies carry greater cost, or fewer false alarms when unnecessary investigation is
more costly.

For RQ3, the study illustrates how the modular workflow supports comparison of process-window
representations on a common evaluation cohort. The resulting error profiles provide a basis for
choosing a configuration according to the intended monitoring objective. This complements the
S7-300 network evaluation with an application to physical process measurements.
...
The studies provide evidence from one S7-300 installation and one SWaT campaign. The S7-300
evaluation separates capture sessions, while SWaT uses later observations for testing. Each
S7-300 attack class contributes one test episode, whose records are correlated; the separate
benign monitoring day provides additional evidence on false alarms. Evaluation across further
installations and operating modes would test transfer.

\paragraph{Operational assessment.}
The SWaT errors total about 4.83 hours of flagged normal observations and 69.6 minutes of missed
anomalous observations. Assessment of individual attacks would match predicted intervals to
attack episodes under a stated overlap rule and report detection delay alongside missed episodes.
Grouping false alarms and reporting their rate per day of normal operation would clarify
operator workload. Deployment precision also depends on the proportion of anomalous observations.

\paragraph{Further validation.}
The measured comparisons concern configurations within the proposed workflow. Comparisons with
simple baselines would test their added value, as motivated by prior industrial detection
studies \cite{wolsing2022simple,bilot2025sometimes}. Repeated training runs would quantify
variability, with uncertainty estimates that account for temporal dependence. Controlled
ablations could isolate the contributions of reconstruction and critic scores. Complete
experimental settings and runtime measurements would support reproduction and deployment planning.

\paragraph{Individual contribution.}
The candidate designed and implemented \nadics{}, its model interface and protocol extractor,
and produced the labelled S7-300 corpus. For SWaT, he developed the multivariate GAN pipeline
and preprocessing variants, conducted and analysed the experiments, and wrote the internal
report. His service-graph contribution covered the concept and measurement design. This account
rests on his attestation and the surviving artefacts; the IUNO deliverables record authorship
at document level \cite{iuno_ap4,iuno_ap7}.
...
inside IUNO the framework reaches 94.50\% attack $F_1$ and 89.56\% macro-$F_1$ over six
classes at a record false-positive rate of 0.484\%, and a benign day with four active PLCs raised
nine alarm episodes, all of
them false. For SWaT process windows, the GAN evaluation reports
91.89\% timestamp recall at a false-positive rate of 5.08\% with 24 PCA
