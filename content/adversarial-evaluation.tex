\chapter{Threat-Guided Evaluation of Adversarial Defences}
\label{chap:advml}
...
The contribution examined here is the SPARTA SAFAIR evaluation framework: a contest with a
hidden evaluation set and a modular benchmark for combinations of attacks and defences
\cite{sparta2021d73, sparta2022d76}. The chapter reconstructs the implemented evaluation from
the project records and compares its measurements with the stated threat model. It then derives
properties of the accuracy-loss score and the binary detector response policy. These deductions
explain what the published aggregate results can establish and which additional observations
would be needed to distinguish competing explanations. They are analytical results of this
chapter, rather than additional experiments from the project.

The empirical evidence comprises the contest submissions and a benchmark evaluation of partner
defences on histopathology images. A short comparison with the project's PDF-malware study
examines how feature constraints alter the evaluation problem. The SD submission descriptions
provide evidence of constant attribute predictions and gradient masking; the benchmark's
unchanging activation-detector accuracy leaves its mechanism unresolved. The contribution is an
assessment of evaluation validity, with no claim to a new attack algorithm or certified defence.
...
\section{AI-System Threat Analysis}
\label{sec:advml:2}

D7.1 surveys AI-system assets and threat taxonomies, and D7.2 describes the defensive mechanisms
studied by the partners \cite{sparta2021d71, sparta2020d72}. The project's relational threat
model combines threat groups from ENISA, asset categories from the Software Engineering
Institute, and attack techniques and knowledge levels from NIST
\cite{damiani2016bigdatathreat, galyardt2020nistir8269comments, tabassi2019nistir8269}.
This combination supplies a common vocabulary for the knowledge base. Its categories must still
be translated into operational assumptions before an experiment can be specified.

In particular, D7.1 Section~3.3.6 distinguishes grey-box cases by knowledge of model architecture,
parameter values, training method or training data. Its agent classification has no separate
axis for an attacker's objective or permitted modifications. This chapter specifies those
conditions for each evaluation, following the security-evaluation framework introduced in
Section~\ref{sec:background:adversarial} \cite{biggio2014security}.

One source definition requires correction. D7.1 Section~2.3.5 describes obtaining model
parameters as an availability attack. This concerns confidentiality of the model. The cited
source defines availability in terms of timely service to authorised users
\cite{auernhammer2019attacks}. The distinction limits the scope of the present evaluation:
the reported outcome is predictive performance under evasion, with no separate measurement of
service availability. The current NIST taxonomy provides the broader terminology
\cite{vassilev2025adversarial}; the earlier draft is cited above as the historical source of
the project's classification.
...
weights $0.2$, $0.4$ and $0.4$ \cite{goodfellow2015explaining, kurakin2017bim, madry2018pgd}.
...
\cite{sparta2021d75, carlini2017robustness}. Table~\ref{tab:advml:designdelivery} records the
change in protocol.
...
The benchmark separates attack execution from the task-specific model and data interface.
D7.3 proposed five modules, which D7.6 reports as implemented: attacks, datasets and models,
together with use cases and a wrapper \cite{sparta2021d73,sparta2022d76}.
Figure~\ref{fig:advml:architecture} redraws the published architecture. A run loads a
checkpoint and the evaluation data, converts the model to the attack-library interface, and
reports task-dependent outputs for the selected attacks.
...
Table~\ref{tab:advml:benchmark} reproduces the partner benchmark results. It measures binary
IDC classification after the defence and, for the two detectors, after the response policy
in Equation~\ref{eq:advml:reversal} \cite{sparta2022d76}. The clean row records the
prediction cost associated with each configuration.
...
The contest toolkit and benchmark are published in the candidate's name
...
relation between clean performance and adversarial robustness. The SPARTA work implemented
a modular interface for attack execution and task-specific model evaluation. Its records
show that a reproducible measurement also needs an explicit prediction objective, a valid
perturbation set and an identified experimental configuration. Independent attack
participation addresses only part of that requirement.
