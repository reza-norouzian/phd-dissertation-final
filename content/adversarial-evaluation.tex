...
the operations permitted on the input. Section~\ref{sec:background:adversarial} defines the
white-box, black-box and grey-box terminology used throughout the dissertation. Under its
grey-box convention, some model information is known and the training process cannot be
influenced. Knowledge of training data and the ability to alter those data are separate
assumptions.
...
The contest defined two tasks, each with an attack track and a defence track: targeted face
re-identification and attribute alteration. In targeted face re-identification, the attacker
modifies a face image so that the classifier assigns the image to a specified target identity. In
attribute alteration, the attacker modifies an image so that the classifier misclassifies a
nominated subset of its forty binary facial attributes. The corresponding defence tracks
evaluated model robustness against the same perturbations \cite{sparta2021d73}. A
participant could attack one task and defend the other, but could not enter both sides of the
same task.
...
therefore records both the input coordinate system and preprocessing.

\subsection{SAFAIR Contest Scoring and Evaluation Protocol}

The SAFAIR contest ranked submissions by the change between clean and perturbed accuracy,
\begin{equation}
...
Attribute imbalance can give a constant predictor high mean accuracy, as reported for SD
\cite{sparta2022d76}. Attribute prevalences and per-attribute outcomes are therefore needed
alongside the aggregate score; the available results do not establish demographic performance
differences.
...
\begin{figure}[tbp]
  ...
  \caption{Benchmark architecture, redrawn after D7.6 Figure~2, p.~22 \cite{sparta2022d76}.
    Box labels follow the original. The five modules separate task definitions and
    data loading from attack execution.}
  \label{fig:advml:architecture}
\end{figure}

Foolbox supplies reference attack implementations \cite{rauber2017foolbox}. The SAFAIR
framework integrates them into a shared evaluation procedure with task-specific adapters.
Identity classification requires a single-label loss, whereas facial attributes require a
multi-label loss. The implementation selects cross-entropy or binary cross-entropy for these
tasks and passes the selected loss to supported attacks
\cite{norouzian2021adversarialbenchmark}. Researchers can thus change the prediction task
while retaining the evaluation loop.

A run loads trained model weights and evaluation data, converts the model to the attack-library
interface, and records clean and attacked accuracies in JSON format. New attack classes extend
a base class and are discovered by the loader; a configuration list excludes named attacks.
PyTorch is the native model interface, and D7.6 describes conversion of TensorFlow or Keras
models \cite{sparta2022d76,norouzian2021adversarialbenchmark}. The public benchmark source
is separate from the contest development toolkit \cite{norouzian2021safairtoolkit}.

Reuse requires compatible prediction criteria and data processing. The multi-label adapter
handles fixed-budget attacks but omits some minimum-distance interfaces. Vicomtech's
histopathology evaluation in Section~\ref{sec:advml:8:benchmark} documents use on a different
dataset and prediction task \cite{sparta2022d76}. Standardisation here concerns the component
interfaces and execution procedure.
...
the raw output need not satisfy that budget \cite{foolbox2021api}.
...
\cite{norouzian2021adversarialbenchmark}. Each output therefore needs an explicit task,
reference label and denominator.

\section{Defence Mechanisms and Integration}
\label{sec:advml:7}

Vicomtech used the benchmark described in Section~\ref{sec:advml:6} to evaluate five defence
configurations documented in D7.2 \cite{sparta2020d72,sparta2022d76}. They instantiate the
three defence strategies introduced in Section~\ref{sec:advml:1}: adversarial training,
transformations of inputs or internal representations, and adversarial-input detection.

The underlying histopathology classifier combines a pretrained VGG16 feature extractor with
a dense prediction network \cite{sparta2020d72,sparta2022d76}. Write its scores as
...
training examples \cite{sparta2022d76}. This method exposes the model to those attacks during
training; resistance to other attacks remains an empirical question.
...
\cite{sparta2020d72}. Figure~\ref{fig:advml:autoencoders} maps the initial and middle
autoencoder designs of D7.2 to the Top AE and Middle AE configurations compared in
Table~\ref{tab:advml:benchmark}.
...
\begin{figure}[tbp]
  ...
  \caption{Autoencoder placement in the partner classifier, redrawn after D7.2 Figures~13
    ...
    $c_\psi$, as in Equation~\ref{eq:advml:compositions}.}
...
\subsection{SD Contest Submissions}
\label{sec:advml:8:sd}

The contest permitted each participant up to three submissions, and D7.6 describes the three
models submitted by SD \cite{sparta2021d73,sparta2022d76}. Although the report introduces
them as re-identification models, their technical descriptions specify forty attribute outputs.
The analysis therefore follows the attribute-model description; their mapping to the official
SD row in Table~\ref{tab:advml:contest} remains unresolved.

The first submission used a convolutional network trained on PGD examples with
...
and contest rules. A constant attribute vector cannot be interpreted as a demonstrated
identity-classification result, and neither the checkpoint nor individual predictions
accompany the reported collapse.

The second submission combines six EfficientNet and three pruned ECA-ResNet models.
...
\end{equation}
For $u\neq0$, thresholding preserves the binary decision while suppressing its direct
derivative. Adaptive or transfer attacks are therefore needed to assess this construction
\cite{athalye2018obfuscated,tramer2020adaptive}.

The second submission achieved approximately 92\% clean accuracy and 79\% under
transferred attacks in the team's development account. The third submission used one pruned
ECA-ResNet, added adversarial training, and retained the threshold operation. Its reported clean
accuracy was approximately 87\%, with 84--85\% under transferred attacks
\cite{sparta2022d76}. Figure~\ref{fig:advml:sd} compares the reported values for these two
submissions.

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\linewidth]{sparta/thesis/sparta-sd-comparison.pdf}
  \caption{Reported clean and transferred-attack accuracies for SD submissions 2 and 3,
    from D7.6 Sections~2.8.1.2--2.8.1.3, pp.~18--19 \cite{sparta2022d76}.
    Submission 1 is omitted because D7.6 reports no comparable transferred-attack accuracy
    for its collapsed model. The 84--85\% interval is an approximate reported range, not a
    confidence interval; these development values are separate from the official contest scores.}
  \label{fig:advml:sd}
\end{figure}

The third submission trades five percentage points of reported clean accuracy for five to
six points of transferred-attack accuracy relative to the second submission. This is a
descriptive comparison between systems: ensemble size and training changed together, and the attacks'
...
DeepFool iteratively linearises the classifier around the current input and moves towards the
closest estimated decision boundary \cite{moosavi2016deepfool}. NewtonFool applies gradient
steps that reduce the original-class softmax probability while bounding the perturbation at
each iteration \cite{jang2017objective}.
...
The near-zero DeepFool results for four configurations require separate interpretation.
DeepFool searches for an adversarial input with low distortion and should be assessed by the
distortion required, or by success under a common valid budget
\cite{carlini2019evaluating}. Neither the norm and stopping settings nor
...
D7.5 reports a separate partner study of PDF-malware evasion that illustrates domain-specific
input constraints \cite{sparta2021d75}. It uses 10,000 benign and 10,000 malicious Contagio
files, with 21 PDFiD tag counts as features. The split assigns 45\% to defender training,
45\% to proxy training and 10\% to adversarial crafting. A neural proxy generates examples
transferred to neural, SVM and random-forest classifiers without target queries.

Tag counts must remain integer-valued and cannot decrease. For an original vector
$v\in\mathbb N_0^{21}$, the reported constraints require
\begin{equation}
  v'\in\mathbb N_0^{21},\qquad v'_j\geq v_j\quad(j=1,\ldots,21).
  \label{eq:advml:pdf-constraints}
\end{equation}
A perturbation bound applies in the standardised representation, followed by inverse
transformation and rounding. These feature constraints do not establish that a corresponding
modified PDF was constructed with its malicious functionality preserved, as required by the
problem-space distinction in Section~\ref{sec:background:adversarial}.
...
\begin{figure}[tbp]
  ...
  \caption{Original iterative-FGSM transfer curves from the partner PDF-malware study,
    D7.5 Figure~15, p.~23 \cite{sparta2021d75}. The horizontal axis is the source's
    $\log_{10}(\delta)$ perturbation budget, distinct from the accuracy loss $\Delta$.
    The published feature-space results contain no uncertainty bands.}
  \label{fig:advml:pdf-transfer}
\end{figure}

The legends identify the SVM, random forest (FST), neural network (ANN) and ensemble (Ensbl).
The prefix r-fgsm denotes adversarial retraining with FGSM examples; Proxy is the attack's
source model. The feature-scattering comparison also retrains the SVM and random forest
with adversarial examples \cite{sparta2021d75}.

Figure~\ref{fig:advml:pdf-transfer} reports model-dependent transfer success across perturbation
budgets. Some curves are non-monotone: they describe the tested searches, not a worst-case
envelope over all admissible perturbations. The comparison supports budget-dependent reporting
under explicit feature constraints, but not a general ranking of defence families. For Android,
the constraints must describe valid application changes and their effect on the detector's
representation.
...
\paragraph{Sampling and uncertainty.} The exact hidden samples are unavailable, and the
histopathology subset size, class proportions and patient-level partitioning are unreported.
Patch counts cannot be treated as independent patient counts,
...
The SAFAIR work addressed RQ4 through a competition and a reusable benchmark framework.
The competition evaluated submitted defences on hidden samples under explicit input
constraints. SD's reported constant-prediction submission exposes a weakness of accuracy-loss
ranking: a constant predictor can obtain zero loss despite limited predictive utility.
Section~\ref{sec:advml:8:metrics} proves that this property persists under any label-preserving
attack. Logit thresholding raises a separate evaluation problem because it obstructs direct
gradients while preserving attribute decisions.

The framework provided a common evaluation procedure with task-specific interfaces.
Vicomtech used it to evaluate histopathology defences, which documents an application outside
the contest's facial-image tasks. The resulting table illustrates why the scoring choice
matters: a configuration with smaller accuracy loss can have lower attacked accuracy.
For detector-based systems, the comparison also depends on the response to an alarm;
unchanged aggregate accuracy alone does not identify the underlying mechanism.

Evaluators should therefore report clean and attacked utility together and test attacks
against the complete defence. Per-example outcomes and valid perturbation budgets are
needed to interpret the scores. The contribution combines a documented evaluation design
with an analysis of its measurement properties. The available aggregates do not establish
a general relation between clean performance and adversarial robustness, and experiment-level
